...
import { GROUP_LIST, IObjects, IUser, NOT_GROUP_LIST, TEAM_TYPE_ID } from '../../models/IUser';
...
const Users:FC = () => {
  ...
  useEffect(() => {
    fetchUsers(searchP, where, IObjects)
  ...
  const [where, setWhere] = useState(' active = 1 AND ' + NOT_GROUP_LIST as string )
  ...
  const columns: ColumnsType<IUser> = [
    ...
    fetchUsers({...searchP, _page: page, _offset: _offset } , where, IObjects) 
  }
  ...
  const selectChanged = (selectChange:any, name:string) =>
    ...
    const onFinish = async (values: any) => { 
      ...
      console.log(where_);
      fetchUsers({...searchP } , where_, IObjects)
      setWhere(where_)
